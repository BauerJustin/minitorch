...
def backpropagate(variable: Variable, deriv: Any) -> None:
    ...
    for var in topological_sort(variable):
        d_output = grads[var.unique_id]
        if var.is_leaf():
            var.accumulate_derivative(d_output)
        else:
            for p_var, grad in var.chain_rule(d_output):
                if p_var.unique_id not in grads:
                    grads[p_var.unique_id] = 0.0
                grads[p_var.unique_id] += grad
